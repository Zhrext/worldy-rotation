...
local function Precombat()
  ...
  if S.SoulFire:IsReady() and (not Player:IsCasting(S.SoulFire)) then
    if Press(S.SoulFire, not Target:IsSpellInRange(S.SoulFire)) then return "soul_fire precombat 4"; end
  end
  -- cataclysm
  if S.Cataclysm:IsCastable() then
    if Press(S.Cataclysm, not Target:IsInRange(40)) then return "cataclysm precombat 6"; end
  end
  -- incinerate
  if S.Incinerate:IsCastable() and (not Player:IsCasting(S.Incinerate)) then
    if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate precombat 8"; end
  end
end
...
local function oGCD()
  if InfernalTime() > 0 or not S.SummonInfernal:IsAvailable() then
    ...
    if S.Berserking:IsCastable() and (FightRemains < (S.SummonInfernal:CooldownRemains() + 12) and (FightRemains > 12) or FightRemains < S.SummonInfernal:CooldownRemains()) then
      if Press(S.Berserking) then return "berserking cds 10"; end
    end
    ...
    if S.BloodFury:IsCastable() and (FightRemains < (S.SummonInfernal:CooldownRemains() + 10 + 15) and (FightRemains > 15) or FightRemains < S.SummonInfernal:CooldownRemains()) then
      if Press(S.BloodFury) then return "blood_fury cds 12"; end
    end
    ...
    if S.Fireblood:IsCastable() and (FightRemains < (S.SummonInfernal:CooldownRemains() + 10 + 8) and (FightRemains > 8) or FightRemains < S.SummonInfernal:CooldownRemains()) then
      if Press(S.Fireblood) then return "fireblood cds 14"; end
    end
  end
end
...
local function Havoc()
  ...
  if S.SoulFire:IsCastable() and (S.SoulFire:CastTime() < VarHavocRemains and Player:SoulShardsP() < 2.5) then
    if Press(S.SoulFire, not Target:IsSpellInRange(S.SoulFire)) then return "soul_fire havoc 4"; end
  end
  ...
  if S.ChannelDemonfire:IsCastable() and (Player:SoulShardsP() < 4.5 and S.RagingDemonfire:TalentRank() == 2 and EnemiesCount8ySplash > 2) then
    if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire havoc 6"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (S.CryHavoc:IsAvailable() and (not S.Inferno:IsAvailable()) and S.ChaosBolt:CastTime() < VarHavocRemains) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt havoc 9"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (S.ChaosBolt:CastTime() < VarHavocRemains and (EnemiesCount8ySplash < 4 - num(S.Inferno:IsAvailable()) + num(S.MadnessoftheAzjAqir:IsAvailable()) - num(S.Inferno:IsAvailable() and (S.RainofChaos:IsAvailable() or S.AvatarofDestruction:IsAvailable()) and Player:BuffUp(S.RainofChaosBuff)))) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt havoc 10"; end
  end
  ...
  if S.RainofFire:IsReady() and (EnemiesCount8ySplash >= 4 - num(S.Inferno:IsAvailable()) + num(S.MadnessoftheAzjAqir:IsAvailable())) then
    if Press(S.RainofFire, not Target:IsSpellInRange(S.Conflagrate)) then return "rain_of_fire havoc 12"; end
  end
  ...
  if S.RainofFire:IsReady() and (EnemiesCount8ySplash > 1 and (S.AvatarofDestruction:IsAvailable() or (S.RainofChaos:IsAvailable() and Player:BuffUp(S.RainofChaosBuff))) and S.Inferno:IsAvailable()) then
    if Press(S.RainofFire, not Target:IsSpellInRange(S.Conflagrate)) then return "rain_of_fire havoc 13"; end
  end
  ...
  if S.Incinerate:IsCastable() and (S.Incinerate:CastTime() < VarHavocRemains) then
    if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate havoc 16"; end
  end
end
...
local function Cleave()
  ...
  if S.ChannelDemonfire:IsCastable() and (S.RagingDemonfire:IsAvailable()) then
    if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire cleave 8"; end
  end
  ...
  if S.SoulFire:IsCastable() and (Player:SoulShardsP() <= 3.5 and (Target:DebuffRemains(S.RoaringBlazeDebuff) > S.SoulFire:CastTime() + S.SoulFire:TravelTime() or (not S.RoaringBlaze:IsAvailable()) and Player:BuffUp(S.BackdraftBuff)) and not VarPoolSoulShards) then
    if Press(S.SoulFire, not Target:IsSpellInRange(S.SoulFire)) then return "soul_fire cleave 10"; end
  end
  ...
  if S.Havoc:IsCastable() and (S.SummonInfernal:CooldownDown() or not S.SummonInfernal:IsAvailable()) then
    ...
    for _, CycleUnit in pairs(Enemies40y) do
      if CycleUnit:GUID() ~= TargetGUID and not CycleUnit:IsFacingBlacklisted() and not CycleUnit:IsUserCycleBlacklisted() then
        WR.CastLeftNameplate(CycleUnit, S.Havoc)
        break
      end
    end
  end
  ...
  if S.ChaosBolt:IsReady() and (InfernalTime() > 0 or BlasphemyTime() > 0 or Player:SoulShardsP() >= 4) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 16"; end
  end
  -- summon_infernal
  if CDsON() and S.SummonInfernal:IsCastable() then
    if Press(S.SummonInfernal) then return "summon_infernal cleave 18"; end
  end
  ...
  if S.ChannelDemonfire:IsCastable() and (S.Ruin:TalentRank() > 1 and (not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable())))) then
    if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire cleave 20"; end
  end
  ...
  if S.Incinerate:IsCastable() and (Player:BuffUp(S.BurntoAshesBuff) and S.Incinerate:CastTime() + S.ChaosBolt:CastTime() < Player:BuffRemains(S.MadnessCBBuff)) then
    if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate cleave 23"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (Player:BuffRemains(S.RainofChaosBuff) > S.ChaosBolt:CastTime()) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 24"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (Player:BuffUp(S.BackdraftBuff) and not VarPoolSoulShards) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 26"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (S.Eradication:IsAvailable() and (not VarPoolSoulShards) and Target:DebuffRemains(S.EradicationDebuff) < S.ChaosBolt:CastTime() and not S.ChaosBolt:InFlight()) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 28"; end
  end
  -- chaos_bolt,if=buff.madness_cb.up
  if S.ChaosBolt:IsReady() and (Player:BuffUp(S.MadnessCBBuff)) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 30"; end
  end
  -- soul_fire,if=soul_shard<=4&talent.mayhem
  if S.SoulFire:IsCastable() and (Player:SoulShardsP() <= 4 and S.Mayhem:IsAvailable()) then
    if Press(S.SoulFire, not Target:IsSpellInRange(S.SoulFire)) then return "soul_fire cleave 32"; end
  end
  ...
  if S.ChannelDemonfire:IsCastable() and (not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable()))) then
    if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire cleave 34"; end
  end
  -- dimensional_rift
  ...
  if S.ChaosBolt:IsReady() and (Player:SoulShardsP() > 3.5) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 38"; end
  end
  ...
  if S.ChaosBolt:IsReady() and ((not VarPoolSoulShards) and (S.SoulConduit:IsAvailable() and (not S.MadnessoftheAzjAqir:IsAvailable()) or not S.Backdraft:IsAvailable())) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 40"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (FightRemains < 5.5 and FightRemains > S.ChaosBolt:CastTime() + S.ChaosBolt:TravelTime() + 0.5) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 42"; end
  end
  ...
  if S.Incinerate:IsCastable() then
    if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate cleave 46"; end
  end
end
...
local function Aoe()
  ...
  if S.RainofFire:IsReady() and (InfernalTime() > 0) then
    if Press(S.RainofFire, not Target:IsSpellInRange(S.Conflagrate)) then return "rain_of_fire aoe 2"; end
  end
  -- rain_of_fire,if=talent.avatar_of_destruction
  if S.RainofFire:IsReady() and (S.AvatarofDestruction:IsAvailable()) then
    if Press(S.RainofFire, not Target:IsSpellInRange(S.Conflagrate)) then return "rain_of_fire aoe 4"; end
  end
  -- rain_of_fire,if=soul_shard=5
  if S.RainofFire:IsReady() and (Player:SoulShardsP() == 5) then
    if Press(S.RainofFire, not Target:IsSpellInRange(S.Conflagrate)) then return "rain_of_fire aoe 6"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (Player:SoulShardsP() > 3.5 - (0.1 * EnemiesCount8ySplash) and not S.RainofFire:IsAvailable()) then
    if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt aoe 8"; end
  end
  -- cataclysm
  if CDsON() and S.Cataclysm:IsCastable() then
  ...
  if S.ChannelDemonfire:IsCastable() and (Target:DebuffRemains(S.ImmolateDebuff) > S.ChannelDemonfire:CastTime() and S.RagingDemonfire:IsAvailable()) then
    if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire aoe 12"; end
  end
  ...
  -- summon_infernal
  if CDsON() and S.SummonInfernal:IsCastable() then
    if Press(S.SummonInfernal) then return "summon_infernal aoe 14"; end
  end
  -- rain_of_fire
  if S.RainofFire:IsReady() then
    if Press(S.RainofFire, not Target:IsSpellInRange(S.Conflagrate)) then return "rain_of_fire aoe 16"; end
  end
  ...
  if S.ChannelDemonfire:IsReady() and (Target:DebuffRemains(S.ImmolateDebuff) > S.ChannelDemonfire:CastTime()) then
    if Press(S.ChannelDemonfire, not Target:IsSpellInRange(S.ChannelDemonfire)) then return "channel_demonfire aoe 17"; end
  end
  ...
  if S.SoulFire:IsCastable() and (Player:BuffUp(S.BackdraftBuff)) then
    if Press(S.SoulFire, not Target:IsSpellInRange(S.SoulFire)) then return "soul_fire aoe 20"; end
  end
  ...
  if S.Incinerate:IsCastable() and (S.FireandBrimstone:IsAvailable() and Player:BuffUp(S.Backdraft)) then
    if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate aoe 22"; end
  end
  ...
  if S.Immolate:IsCastable() and (Target:DebuffRefreshable(S.ImmolateDebuff)) then
    if Press(S.Immolate, not Target:IsSpellInRange(S.Immolate)) then return "immolate aoe 28"; end
  end
  -- incinerate
  if S.Incinerate:IsCastable() then
    if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate aoe 30"; end
  end
end
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.ChannelDemonfire:IsReady() and S.RagingDemonfire:IsAvailable() then
      if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire main 7"; end
    end
    ...
    if S.SoulFire:IsCastable() and (Player:SoulShardsP() <= 3.5 and (Target:DebuffRemains(S.RoaringBlazeDebuff) > S.SoulFire:CastTime() + S.SoulFire:TravelTime() or (not S.RoaringBlaze:IsAvailable()) and Player:BuffUp(S.BackdraftBuff))) then
      if Press(S.SoulFire, not Target:IsSpellInRange(S.SoulFire)) then return "soul_fire main 8"; end
    end
    ...
    if S.Immolate:IsCastable() and (((Target:DebuffRefreshable(S.ImmolateDebuff) and S.InternalCombustion:IsAvailable()) or Target:DebuffRemains(S.ImmolateDebuff) < 3) and ((not S.Cataclysm:IsAvailable()) or S.Cataclysm:CooldownRemains() > Target:DebuffRemains(S.ImmolateDebuff)) and ((not S.SoulFire:IsAvailable()) or S.SoulFire:CooldownRemains() + S.SoulFire:CastTime() > Target:DebuffRemains(S.ImmolateDebuff))) then
      if Press(S.Immolate, not Target:IsSpellInRange(S.Immolate)) then return "immolate main 10"; end
    end
    ...
    -- chaos_bolt,if=pet.infernal.active|pet.blasphemy.active|soul_shard>=4
    if S.ChaosBolt:IsReady() and (InfernalTime() > 0 or BlasphemyTime() > 0 or Player:SoulShardsP() >= 4) then
      if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt main 12"; end
    end
    -- summon_infernal
    if CDsON() and S.SummonInfernal:IsCastable() then
      if Press(S.SummonInfernal) then return "summon_infernal main 14"; end
    end
    ...
    if S.ChannelDemonfire:IsCastable() and (S.Ruin:TalentRank() > 1 and not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable())) and Target:DebuffRemains(S.ImmolateDebuff) > S.ChannelDemonfire:CastTime()) then
      if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire main 16"; end
    end
    ...
    --incinerate,if=buff.burn_to_ashes.up&cast_time+action.chaos_bolt.cast_time<buff.madness_cb.remains
    if S.Incinerate:IsCastable() and (Player:BuffUp(S.BurntoAshesBuff) and S.Incinerate:CastTime() + S.ChaosBolt:CastTime() < Player:BuffRemains(S.MadnessCBBuff)) then
      if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate aoe 29"; end
    end
    -- chaos_bolt,if=buff.rain_of_chaos.remains>cast_time
    if S.ChaosBolt:IsReady() and (Player:BuffRemains(S.RainofChaosBuff) > S.ChaosBolt:CastTime()) then
      if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt main 30"; end
    end
    ...
    if S.ChaosBolt:IsReady() and (Player:BuffUp(S.Backdraft) and (not S.Eradication:IsAvailable()) and not S.MadnessoftheAzjAqir:IsAvailable()) then
      if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt main 32"; end
    end
    -- chaos_bolt,if=buff.madness_cb.up
    if S.ChaosBolt:IsReady() and (Player:BuffUp(S.MadnessCBBuff)) then
      if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt cleave 36"; end
    end
    ...
    if S.ChannelDemonfire:IsCastable() and (not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable())) and Target:DebuffRemains(S.ImmolateDebuff) > S.ChannelDemonfire:CastTime()) then
      if Press(S.ChannelDemonfire, not Target:IsInRange(40)) then return "channel_demonfire cleave 38"; end
    end
    -- dimensional_rift
    if CDsON() and S.DimensionalRift:IsCastable() then
    ...
    -- chaos_bolt,if=soul_shard>3.5
    if S.ChaosBolt:IsReady() and (Player:SoulShardsP() >= 3.5) then
      if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt main 42"; end
    end
    ...
    if S.ChaosBolt:IsReady() and (S.SoulConduit:IsAvailable() and (not S.MadnessoftheAzjAqir:IsAvailable()) or not S.Backdraft:IsAvailable()) then
      if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt main 44"; end
    end
    -- chaos_bolt,if=time_to_die<5&time_to_die>cast_time+travel_time
    -- Note: Added a buffer of 0.5s
    if S.ChaosBolt:IsReady() and (FightRemains < 5.5 and FightRemains > S.ChaosBolt:CastTime() + S.ChaosBolt:TravelTime() + 0.5) then
      if Press(S.ChaosBolt, not Target:IsSpellInRange(S.ChaosBolt)) then return "chaos_bolt main 46"; end
    end
    -- conflagrate,if=charges>(max_charges-1)|time_to_die<gcd*charges
    ...
    -- incinerate
    if S.Incinerate:IsCastable() then
      if Press(S.Incinerate, not Target:IsSpellInRange(S.Incinerate)) then return "incinerate main 50"; end
    end
  end
end
