...
local Settings = {
  ...
  Commons = WR.GUISettings.APL.Shaman.Commons,
  Enhancement = WR.GUISettings.APL.Shaman.Enhancement
}
...
local function Precombat()
  ...
  -- Note: These variables just exclude these three trinkets from the generic use_items. We'll just use WR's OnUseExcludes instead.
  -- snapshot_stats
end
...
local function APL()
  ...
  if Everyone.TargetIsValid() then
    ...
    if (Settings.Commons.Enabled.Trinkets) then
      ...
      -- Note: These variables just exclude the above three trinkets from the generic use_items. We'll just use WR's OnUseExcludes instead.
      -- use_items
...
local function AutoBind()
  -- Spell Binds
  WR.Bind(S.FrostShock)
  --WR.Bind(S.HealingSurge)
  WR.Bind(S.LavaBurst)
  WR.Bind(S.LightningBolt)
  WR.Bind(S.LightningShield)
  --WR.Bind(S.Purge)
  --WR.Bind(S.SpiritWalk)
  --WR.Bind(S.WaterWalking)
  WR.Bind(S.WindShear)
  WR.Bind(S.CrashLightning)
  WR.Bind(S.DoomWinds)
  WR.Bind(S.FeralSpirit)
  WR.Bind(S.IceStrike)
  WR.Bind(S.LavaLash)
  WR.Bind(S.PrimordialWave)
  WR.Bind(S.Stormstrike)
  WR.Bind(S.Sundering)
  WR.Bind(S.WindfuryWeapon)
  WR.Bind(S.FlamentongueWeapon)
  WR.Bind(S.EarthElemental)
  WR.Bind(S.ChainLightning)
  WR.Bind(S.FlameShock)
end

local function Init()
  S.FlameShockDebuff:RegisterAuraTracking()

  WR.Print("Enhance Shaman by Worldy")
  AutoBind()
end


WR.SetAPL(263, APL, Init)
