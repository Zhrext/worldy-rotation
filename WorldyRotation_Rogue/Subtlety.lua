...
local function ShD_Combo_Points ()
  ...
  elseif MeleeEnemies10yCount == 3 then
   return ComboPointsDeficit >= 2
  else
...
local function Finish (ReturnSpellOnly, StealthSpell)
  ...
  if S.SecretTechnique:IsCastable() and ShadowDanceBuff and (Player:BuffStack(S.DanseMacabreBuff) >= 3 or not S.DanseMacabre:IsAvailable()) then
    if ReturnSpellOnly then
      ...
      if S.SecretTechnique:IsReady() and WR.Cast(S.SecretTechnique) then return "Cast Secret Technique" end
  ...
  if S.BlackPowder:IsCastable() and not PriorityRotation and MeleeEnemies10yCount >= (3 - BoolToInt(S.DarkBrew:IsAvailable())) then
    if ReturnSpellOnly then
...
local function Stealthed (ReturnSpellOnly, StealthSpell)
  ...
  if ShadowstrikeIsCastable and (StealthBuff or VanishBuffCheck) and (MeleeEnemies10yCount < 4 or PriorityRotation) then
    if ReturnSpellOnly then
      ...
      if WR.Cast(S.Shadowstrike) then return "Cast Shadowstrike (Stealth)" end
    end
  end
  ...
  if (ComboPointsDeficit == 2 or ComboPointsDeficit == 3) and (PremeditationBuff or EffectiveComboPoints < 7) and (MeleeEnemies10yCount <= 8 or S.LingeringShadow:IsAvailable()) then
    ...
    if S.Gloomblade:IsCastable() then
      if ReturnSpellOnly then
        ...
        if WR.CastQueue(S.Gloomblade, S.Stealth) then return "Cast Gloomblade (Stealth)" end
      end
    elseif S.Backstab:IsCastable() and S.DanseMacabre:IsAvailable() and Player:BuffStack(S.DanseMacabreBuff) <= 2 then
      if ReturnSpellOnly then
        -- If calling from a Stealth macro, we don't need the PV suggestion since it's already a macro cast
        if StealthSpell then
          return S.Backstab
        ...
        if WR.CastQueue(S.Backstab, S.Stealth) then return "Cast Backstab (Stealth)" end
      end
    end
  end
  ...
  if Player:BuffStack(S.PerforatedVeinsBuff) >= 5 and MeleeEnemies10yCount < 3 then
    if S.Gloomblade:IsCastable() then
      if ReturnSpellOnly then
        ...
        if WR.CastQueue(S.Gloomblade, S.PerforatedVeins) then return "Cast Gloomblade (Stealth PV)" end
      end
    elseif S.Backstab:IsCastable() then
      if ReturnSpellOnly then
        ...
        if WR.CastQueue(S.Backstab, S.PerforatedVeins) then return "Cast Backstab (Stealth PV)" end
      end
    end
  end
  ...
  if ShadowstrikeIsCastable and not Player:StealthUp(true, false) and not StealthSpell and Player:BuffUp(S.SepsisBuff) and MeleeEnemies10yCount < 4 then
    if ReturnSpellOnly then
      ...
      if WR.Cast(S.Shadowstrike) then return "Cast Shadowstrike (Sepsis)" end
    end
  end
  ...
  if WR.AoEON() and S.ShurikenStorm:IsCastable()
    ...
    if ReturnSpellOnly then
      ...
      if WR.Cast(S.ShurikenStorm) then return "Cast Shuriken Storm" end
    end
  end
  ...
  if ShadowstrikeIsCastable and (Target:DebuffRemains(S.FindWeaknessDebuff) < 1 or S.SymbolsofDeath:CooldownRemains() < 18
    ...
    if ReturnSpellOnly then
      ...
      if WR.Cast(S.Shadowstrike) then return "Cast Shadowstrike (FW Refresh)" end
    end
  end
  ...
  if ShadowstrikeIsCastable then
    if ReturnSpellOnly then
      ...
      if WR.Cast(S.Shadowstrike) then return "Cast Shadowstrike 2" end
    end
  end
...
local function StealthMacro (StealthSpell, EnergyThreshold)
  ...
  if StealthSpell == S.Vanish and (not Settings.Subtlety.StealthMacro.Vanish or not MacroAbility) then
    if WR.Cast(S.Vanish, Settings.Commons.OffGCDasOffGCD.Vanish) then return "Cast Vanish" end
    return false
  ...
  elseif StealthSpell == S.ShadowDance and (not Settings.Subtlety.StealthMacro.ShadowDance or not MacroAbility) then
    if WR.Cast(S.ShadowDance, Settings.Subtlety.OffGCDasOffGCD.ShadowDance) then return "Cast Shadow Dance" end
    return false
  end
  ...
  if MacroTable[1] == S.ShadowDance and MacroTable[2] == S.DeathfromAbove then
    ShouldReturn = WR.CastQueue(MacroTable[2], MacroTable[1])
    if ShouldReturn then return "| " .. MacroTable[1]:Name() end
  else
    ShouldReturn = WR.CastQueue(unpack(MacroTable))
...
local function CDs ()
  if Player:BuffUp(S.ShurikenTornado) then
    ...
    if S.SymbolsofDeath:IsCastable() and S.ShadowDance:IsCastable() and not Player:BuffUp(S.SymbolsofDeath) and not Player:BuffUp(S.ShadowDance) then
      if WR.CastQueue(S.SymbolsofDeath, S.ShadowDance) then return "Dance + Symbols (during Tornado)" end
    elseif S.SymbolsofDeath:IsCastable() and not Player:BuffUp(S.SymbolsofDeath) then
      if WR.Cast(S.SymbolsofDeath) then return "Cast Symbols of Death (during Tornado)" end
    elseif S.ShadowDance:IsCastable() and not Player:BuffUp(S.ShadowDanceBuff) then
      if WR.Cast(S.ShadowDance) then return "Cast Shadow Dance (during Tornado)" end
    end
  end
  ...
  if S.ShurikenTornado:IsCastable() and MeleeEnemies10yCount <= 1 and SnDCondition and S.SymbolsofDeath:CooldownUp() and S.ShadowDance:Charges() >= 1
    ...
    if Player:Energy() >= 60 then
      if WR.Cast(S.ShurikenTornado, Settings.Subtlety.GCDasOffGCD.ShurikenTornado) then return "Cast Shuriken Tornado" end
    elseif not S.ShadowFocus:IsAvailable() then
      if WR.Cast(S.ShurikenTornado) then return "Pool for Shuriken Tornado" end
    end
  end
  if TargetInMeleeRange then
    ...
    if S.SymbolsofDeath:IsCastable() and SnDCondition and (not S.Flagellation:IsAvailable() or S.Flagellation:CooldownRemains() > 10
      ...
      if WR.Cast(S.SymbolsofDeath, Settings.Subtlety.OffGCDasOffGCD.SymbolsofDeath) then return "Cast Symbols of Death" end
    end
  end
  ...
  if WR.CDsON() then
    ...
    if S.ShadowBlades:IsCastable() and SnDCondition and ComboPointsDeficit >= 2 then
      if WR.Cast(S.ShadowBlades, Settings.Subtlety.OffGCDasOffGCD.ShadowBlades) then return "Cast Shadow Blades" end
    end
    ...
    if S.ShurikenTornado:IsReady() and SnD_Condition and Player:BuffUp(S.SymbolsofDeath) and ComboPoints <= 2
      and (not Player:BuffUp(S.PremeditationBuff) or MeleeEnemies10yCount > 4) then
      if WR.Cast(S.ShurikenTornado, Settings.Subtlety.GCDasOffGCD.ShurikenTornado) then return "Cast Shuriken Tornado (SF)" end
    end
    ...
    if S.ThistleTea:IsCastable() and not Player:BuffUp(S.ThistleTea)
      and (S.SymbolsofDeath:CooldownRemains() >= 3 and (Player:EnergyDeficit() >= 100 or S.ThistleTea:ChargesFractional() >= 2.75 and Player:EnergyDeficit() >= 40)
        or Player:BuffUp(S.ShadowDanceBuff) and MeleeEnemies10yCount >= 2) then
      if WR.Cast(S.ThistleTea, Settings.Commons.OffGCDasOffGCD.ThistleTea) then return "Cast Thistle Tea" end
    end
...
local function Stealth_CDs (EnergyThreshold)
  if WR.CDsON() and S.ShadowDance:TimeSinceLastDisplay() > 0.3 and S.Shadowmeld:TimeSinceLastDisplay() > 0.3 and not Player:IsTanking(Target) then
    ...
    if S.Vanish:IsCastable() and not ShD_Threshold() and ComboPointsDeficit > 1 then
    ...
    if S.Shadowmeld:IsCastable() and TargetInMeleeRange and not Player:IsMoving()
      ...
      if Player:Energy() < 40 then
        if WR.Cast(S.Shadowmeld, Player:EnergyTimeToX(40)) then return "Pool for Shadowmeld" end
      end
  ...
  if TargetInMeleeRange and S.ShadowDance:IsCastable() and S.ShadowDance:Charges() >= 1
    ...
    and (WR.CDsON() or (S.ShadowDance:ChargesFractional() >= Settings.Subtlety.ShDEcoCharge - (not S.ShadowDanceTalent:IsAvailable() and 0.75 or 0))) then
    ...
    if MayBurnShadowDance() and (ShD_Combo_Points() and HL.BossFilteredFightRemains("<", S.SymbolsofDeath:CooldownRemains())
      or not S.ShadowDanceTalent:IsAvailable() and Target:DebuffUp(S.Rupture) and MeleeEnemies10yCount <= 4) then
      ShouldReturn = StealthMacro(S.ShadowDance, EnergyThreshold)
...
local function Build (EnergyThreshold)
  ...
  if WR.AoEON() and S.ShurikenStorm:IsCastable() and MeleeEnemies10yCount >= 2 + BoolToInt(Player:BuffUp(S.LingeringShadowBuff) or Player:BuffUp(S.PerforatedVeinsBuff)) then
...
local function APL ()
  ...
  if not Player:AffectingCombat() then
    ...
    if Everyone.TargetIsValid() and (Target:IsSpellInRange(S.Shadowstrike) or TargetInMeleeRange) then
      ...
      if Player:StealthUp(true, true) then
        ...
        if PoolingAbility then -- To avoid pooling icon spam
          if type(PoolingAbility) == "table" and #PoolingAbility > 1 then
            if WR.CastQueuePooling(nil, unpack(PoolingAbility)) then return "Stealthed Macro Cast or Pool (OOC): ".. PoolingAbility[1]:Name() end
          else
            if WR.Cast(PoolingAbility) then return "Stealthed Cast or Pool (OOC): "..PoolingAbility:Name() end
          end
        end
      ...
      elseif S.Backstab:IsCastable() then
        if WR.Cast(S.Backstab) then return "Cast Backstab (OOC)" end
      end
    end
    return
  end
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.SliceandDice:IsCastable() and MeleeEnemies10yCount < Rogue.CPMaxSpend() and HL.FilteredFightRemains(MeleeEnemies10y, ">", 6)
      ...
      if S.SliceandDice:IsReady() and WR.Cast(S.SliceandDice) then return "Cast Slice and Dice (Low Duration)" end
      SetPoolingFinisher(S.SliceandDice)
    end
    ...
    if Player:StealthUp(true, true) then
      ...
      if PoolingAbility then -- To avoid pooling icon spam
        if type(PoolingAbility) == "table" and #PoolingAbility > 1 then
          if WR.CastQueuePooling(nil, unpack(PoolingAbility)) then return "Stealthed Macro " .. PoolingAbility[1]:Name() .. "|" .. PoolingAbility[2]:Name() end
        else
          ...
          if Player:BuffUp(S.ShurikenTornado) and ComboPoints ~= Player:ComboPoints()
            ...
            if WR.CastQueuePooling(nil, S.ShurikenTornado, PoolingAbility) then return "Stealthed Tornado Cast  " .. PoolingAbility:Name() end
          else
            if WR.Cast(PoolingAbility) then return "Stealthed Cast " .. PoolingAbility:Name() end
          end
        end
      end
      WR.Cast(S.PoolEnergy)
    ...
    if WR.CDsON() then
      ...
      if S.ArcaneTorrent:IsReady() and TargetInMeleeRange and Player:EnergyDeficitPredicted() > 15 + Player:EnergyRegen() then
        if WR.Cast(S.ArcaneTorrent, Settings.Commons.GCDasOffGCD.Racials) then return "Cast Arcane Torrent" end
      end
      -- actions+=/arcane_pulse
      if S.ArcanePulse:IsReady() and TargetInMeleeRange then
        if WR.Cast(S.ArcanePulse, Settings.Commons.GCDasOffGCD.Racials) then return "Cast Arcane Pulse" end
      end
      -- actions+=/lights_judgment
      if S.LightsJudgment:IsReady() then
        if WR.Cast(S.LightsJudgment, Settings.Commons.GCDasOffGCD.Racials) then return "Cast Lights Judgment" end
      end
      -- actions+=/bag_of_tricks
      if S.BagofTricks:IsReady() then
        if WR.Cast(S.BagofTricks, Settings.Commons.GCDasOffGCD.Racials) then return "Cast Bag of Tricks" end
      end
    end
    ...
    if PoolingAbility and TargetInMeleeRange then
      if type(PoolingAbility) == "table" and #PoolingAbility > 1 then
        if WR.CastQueuePooling(Player:EnergyTimeToX(PoolingEnergy), unpack(PoolingAbility)) then return "Macro pool towards ".. PoolingAbility[1]:Name() .. " at " .. PoolingEnergy end
      elseif PoolingAbility:IsCastable() then
        PoolingEnergy = mathmax(PoolingEnergy, PoolingAbility:Cost())
        if WR.Cast(PoolingAbility, Player:EnergyTimeToX(PoolingEnergy)) then return "Pool towards: " .. PoolingAbility:Name() .. " at " .. PoolingEnergy end
      end
    end
    ...
    if S.ShurikenToss:IsCastable() and Target:IsInRange(30) and not TargetInAoERange and not Player:StealthUp(true, true) and not Player:BuffUp(S.Sprint)
      ...
      if WR.Cast(S.ShurikenToss) then return "Cast Shuriken Toss" end
    end
  end
end
