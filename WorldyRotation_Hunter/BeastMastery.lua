...
local function Precombat()
  ...
  if S.TarTrap:IsCastable() and (SoulForgeEmbersEquipped or NessingwarysEquipped) then
    if Cast(S.TarTrap) then return "tar_trap precombat 4"; end
  end
  ...
  if S.BestialWrath:IsCastable() and CDsON() and ((not S.ScentOfBlood:IsAvailable()) and (not SoulForgeEmbersEquipped)) then
    if Cast(S.BestialWrath) then return "bestial_wrath precombat 6"; end
  end
...
local function CDs()
  ...
  if S.AncestralCall:IsCastable() and (S.BestialWrath:CooldownRemains() > 30) then
    if Cast(S.AncestralCall) then return "ancestral_call cds 2"; end
  end
  ...
  if S.LightsJudgment:IsCastable() and (Pet:BuffRemains(S.FrenzyPetBuff) > GCDMax or Pet:BuffDown(S.FrenzyPetBuff)) then
    if Cast(S.LightsJudgment) then return "lights_judgment cds 10"; end
  end
...
local function Cleave()
  ...
  if S.AspectoftheWild:IsCastable() and CDsON() then
    if Cast(S.AspectoftheWild) then return "aspect_of_the_wild cleave 2"; end
  end
  ...
  if S.BarbedShot:IsCastable() then
    ...
    if EvaluateBarbedShotCycleCondition1(Target) then
      if Cast(S.BarbedShot) then return "barbed_shot @target cleave 4"; end
    end
  end
  ...
  if S.TarTrap:IsCastable() and (SoulForgeEmbersEquipped and S.Flare:CooldownRemains() < Player:GCD()) then
    if Cast(S.TarTrap) then return "tar_trap cleave 12"; end
  end
  ...
  if S.Flare:IsCastable() and not S.TarTrap:CooldownUp() and SoulForgeEmbersEquipped then
    if Cast(S.Flare) then return "flare cleave 14"; end
  end
  ...
  if S.DeathChakram:IsCastable() and (Player:Focus() + Player:FocusCastRegen(S.DeathChakram:ExecuteTime()) < Player:FocusMax()) then
    if Cast(S.DeathChakram) then return "death_chakram cleave 16"; end
  end
  ...
  if S.WildSpirits:IsCastable() and CDsON() then
    if Cast(S.WildSpirits) then return "wild_spirits cleave 18"; end
  end
  ...
  if S.BarbedShot:IsCastable() then
    ...
    if EvaluateBarbedShotCycleCondition2(Target) then
      if Cast(S.BarbedShot) then return "barbed_shot @target cleave 20"; end
    end
  end
  ...
  if S.BestialWrath:IsCastable() and CDsON() then
    if Cast(S.BestialWrath) then return "bestial_wrath cleave 22"; end
  end
  ...
  if S.ResonatingArrow:IsCastable() and CDsON() then
    if Cast(S.ResonatingArrow) then return "resonating_arrow cleave 24"; end
  end
  ...
  if S.Stampede:IsCastable() and CDsON() and (Player:BuffUp(S.AspectoftheWildBuff) or Target:TimeToDie() < 15) then
    if Cast(S.Stampede) then return "stampede cleave 26"; end
  end
  ...
  if S.WailingArrow:IsReady() and CDsON() and (Pet:BuffRemains(S.FrenzyPetBuff) > S.WailingArrow:ExecuteTime()) then
    if Cast(S.WailingArrow) then return "wailing_arrow cleave 28"; end
  end
  -- flayed_shot
  if S.FlayedShot:IsCastable() then
    if Cast(S.FlayedShot) then return "flayed_shot cleave 30"; end
  end
  -- kill_shot
  if S.KillShot:IsCastable() then
    if Cast(S.KillShot) then return "kill_shot cleave 32"; end
  end
  -- chimaera_shot
  if S.ChimaeraShot:IsCastable() then
    if Cast(S.ChimaeraShot) then return "chimaera_shot cleave 34"; end
  end
  -- bloodshed
  ...
  if S.AMurderofCrows:IsReady() and CDsON() then
    if Cast(S.AMurderofCrows) then return "a_murder_of_crows cleave 38"; end
  end
  ...
  if S.Barrage:IsReady() and Pet:BuffRemains(S.BeastCleavePetBuff) > S.Barrage:ExecuteTime() and Player:BuffRemains(S.BeastCleaveBuff) > S.Barrage:ExecuteTime() then
    if Cast(S.Barrage) then return "barrage cleave 40"; end
  end
  ...
  if S.BagofTricks:IsCastable() and (Player:BuffDown(S.BestialWrathBuff) or Target:TimeToDie() < 5) then
    if Cast(S.BagofTricks) then return "bag_of_tricks cleave 44"; end
  end
  -- dire_beast
  if S.DireBeast:IsReady() then
    if Cast(S.DireBeast) then return "dire_beast cleave 46"; end
  end
  ...
  if S.BarbedShot:IsCastable() then
    ...
    if EvaluateBarbedShotCycleCondition3(Target) then
      if Cast(S.BarbedShot) then return "barbed_shot @target cleave 48"; end
    end
  end
  ...
  if S.ArcaneTorrent:IsCastable() and CDsON() and ((Player:Focus() + Player:FocusRegen() + 30) < Player:FocusMax()) then
    if Cast(S.ArcaneTorrent) then return "arcane_torrent cleave 56"; end
  end
end
...
local function ST()
  ...
  if S.BarbedShot:IsCastable() and (Pet:BuffUp(S.FrenzyPetBuff) and Pet:BuffRemains(S.FrenzyPetBuff) <= GCDMax or Player:BuffUp(S.WildSpiritsBuff) and S.BarbedShot:ChargesFractional() > 1.4 and ElderAntlersEquipped) then
    if Cast(S.BarbedShot) then return "barbed_shot st 4"; end
  end
  ...
  if S.TarTrap:IsCastable() and (SoulForgeEmbersEquipped and Target:BuffDown(S.SoulforgeEmbersDebuff) and S.Flare:CooldownRemains() < Player:GCD()) then
    if Cast(S.TarTrap) then return "tar_trap st 6"; end
  end
  -- flare,if=tar_trap.up&runeforge.soulforge_embers
  -- TODO: Find a way to track traps
  if S.Flare:IsCastable() and not S.TarTrap:CooldownUp() and SoulForgeEmbersEquipped then
    if Cast(S.Flare) then return "flare st 8"; end
  end
  -- bloodshed
  if S.Bloodshed:IsCastable() and CDsON() then
  ...
  if S.WildSpirits:IsCastable() and CDsON() then
    if Cast(S.WildSpirits) then return "wild_spirits st 12"; end
  end
  -- flayed_shot
  if S.FlayedShot:IsCastable() then
    if Cast(S.FlayedShot) then return "flayed_shot st 14"; end
  end
  -- kill_shot
  if S.KillShot:IsCastable() then
    if Cast(S.KillShot) then return "kill_shot st 16"; end
  end
  ...
  if S.WailingArrow:IsReady() and (Pet:BuffRemains(S.FrenzyPetBuff) > S.WailingArrow:ExecuteTime() and (S.ResonatingArrow:CooldownRemains() < Player:GCD() or CovenantID ~= 1) or Target:TimeToDie() < 5) then
    if Cast(S.WailingArrow) then return "wailing_arrow st 18"; end
  end
  ...
  if S.BarbedShot:IsCastable() and ((S.BestialWrath:CooldownRemains() < 12 * S.BarbedShot:ChargesFractional() + GCDMax and S.ScentOfBlood:IsAvailable()) or (S.BarbedShot:FullRechargeTime() < GCDMax and bool(S.BestialWrath:CooldownRemains())) or Target:TimeToDie() < 9) then
    if Cast(S.BarbedShot) then return "barbed_shot st 20"; end
  end
  -- death_chakram,if=focus+cast_regen<focus.max
  if S.DeathChakram:IsCastable() and (Player:Focus() + Player:FocusCastRegen(S.DeathChakram:ExecuteTime()) < Player:FocusMax()) then
    if Cast(S.DeathChakram) then return "death_chakram st 22"; end
  end
  ...
  if S.Stampede:IsCastable() and CDsON() and ((Player:BuffUp(S.AspectoftheWildBuff) and Player:BuffUp(S.BestialWrathBuff)) or Target:TimeToDie() < 15) then
    if Cast(S.Stampede) then return "stampede st 24"; end
  end
  -- a_murder_of_crows
  if S.AMurderofCrows:IsReady() and CDsON() then
    if Cast(S.AMurderofCrows) then return "a_murder_of_crows st 26"; end
  end
  ...
  if S.ResonatingArrow:IsCastable() and CDsON() and (Player:BuffUp(S.BestialWrathBuff) or Target:TimeToDie() < 10) then
    if Cast(S.ResonatingArrow) then return "resonating_arrow st 28"; end 
  end
  ...
  if S.BestialWrath:IsCastable() and CDsON() and (not S.WildSpirits:IsAvailable() or S.WildSpirits:CooldownRemains() > 15 or CovenantID == 1 and (S.ResonatingArrow:CooldownRemains() < 5 or S.ResonatingArrow:CooldownRemains() > 20) or Target:TimeToDie() < 15 or (CovenantID ~= 3 and CovenantID ~= 1)) then
    if Cast(S.BestialWrath) then return "bestial_wrath st 30"; end
  end
  -- chimaera_shot
  if S.ChimaeraShot:IsCastable() then
    if Cast(S.ChimaeraShot) then return "chimaera_shot st 32"; end
  end
  -- kill_command
  ...
  -- bag_of_tricks,if=buff.bestial_wrath.down|target.time_to_die<5
  if S.BagofTricks:IsCastable() and (Player:BuffDown(S.BestialWrathBuff) or Target:TimeToDie() < 5) then
    if Cast(S.BagofTricks) then return "bag_of_tricks st 36"; end
  end
  -- dire_beast
  if S.DireBeast:IsReady() then
    if Cast(S.DireBeast) then return "dire_beast st 38"; end
  end
  ...
  if S.CobraShot:IsReady() and (((Player:Focus() - S.CobraShot:Cost() + Player:FocusRegen() * (S.KillCommand:CooldownRemains() - 1) > S.KillCommand:Cost())or (S.KillCommand:CooldownRemains() > 1 + GCDMax)) or ((Player:BuffUp(S.BestialWrathBuff) or Player:BuffUp(S.NessingwarysTrappingBuff)) and not QaplaEredunWarOrderEquipped) or Target:TimeToDie() < 3) then
    if Cast(S.CobraShot) then return "cobra_shot st 40"; end
  end
  ...
  if S.BarbedShot:IsCastable() and (Player:BuffUp(S.WildSpiritsBuff) or S.BarbedShot:ChargesFractional() > 1.2 and S.Bloodletting:ConduitEnabled()) then
    if Cast(S.BarbedShot) then return "barbed_shot st 42"; end
  end
  ...
  if S.ArcanePulse:IsCastable() and CDsON() and (Player:BuffDown(S.BestialWrathBuff) or Target:TimeToDie() < 5) then
    if Cast(S.ArcanePulse) then return "arcane_pulse st 44"; end
  end
  -- tar_trap,if=runeforge.soulforge_embers|runeforge.nessingwarys_trapping_apparatus
  -- freezing_trap,if=runeforge.nessingwarys_trapping_apparatus
  ...
  if S.ArcaneTorrent:IsCastable() and CDsON() and ((Player:Focus() + Player:FocusRegen() + 15) < Player:FocusMax()) then
    if Cast(S.ArcaneTorrent) then return "arcane_torrent st 50"; end
  end
end
...
local function Trinkets()
  ...
  -- variable,name=strong_sync_remains,op=setif,condition=covenant.kyrian,value=cooldown.resonating_arrow.remains<?cooldown.aspect_of_the_wild.remains_guess,value_else=cooldown.aspect_of_the_wild.remains_guess,if=buff.aspect_of_the_wild.down
  -- variable,name=strong_sync_remains,op=setif,condition=covenant.kyrian,value=cooldown.resonating_arrow.remains,value_else=cooldown.aspect_of_the_wild.remains_guess,if=buff.aspect_of_the_wild.up
  -- variable,name=sync_remains,op=setif,condition=covenant.kyrian,value=cooldown.resonating_arrow.remains>?cooldown.aspect_of_the_wild.remains_guess,value_else=cooldown.aspect_of_the_wild.remains_guess
...
local function APL()
  ...
  if AoEON() then
    ...
    PetEnemiesMixedyCount = (PetCleaveAbility and #Player:GetEnemiesInSpellActionRange(PetCleaveAbility)) or Target:GetEnemiesInSplashRangeCount(8) -- Beast Cleave (tWRough Multi-Shot)
  else
    Enemies40y = {}
...
local function AutoBind()
  ...
  WR.Bind(S.FlayedShot)
  WR.Bind(S.Flare)
  WR.Bind(S.KillCommand)
  WR.Bind(S.KillShot)
  WR.Bind(S.MendPet)
  WR.Bind(S.MultiShot)
  WR.Bind(S.ResonatingArrow)
  WR.Bind(S.RevivePet)
  WR.Bind(S.Stampede)
  WR.Bind(S.TarTrap)
  WR.Bind(S.WailingArrow)
  WR.Bind(S.WildSpirits)
end
