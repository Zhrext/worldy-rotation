...
function (self, Range, AoESpell, ThisUnit, BypassRecovery, Offset)
  ...
  if self == SpellMM.SummonPet then
    return (not Pet:IsActive() and not WR.GUISettings.APL.Hunter.Marksmanship.UseLoneWolf) and BaseCheck
...
function (self, Range, AoESpell, ThisUnit, BypassRecovery, Offset)
  ...
  elseif self == SpellMM.ExplosiveShot then
    local Enemies10ySplash = Target:GetEnemiesInSplasWRange(10)
...
function (self, Range, AoESpell, ThisUnit, BypassRecovery, Offset)
  ...
  elseif self == SpellSV.MendPet then
    return (not Pet:IsDeadOrGhost()) and Pet:HealthPercentage() <= WR.GUISettings.APL.Hunter.Commons2.MendPetHighHP and BaseCheck
  elseif self == SpellSV.AspectoftheEagle then
    return WR.GUISettings.APL.Hunter.Survival.AspectOfTheEagle and BaseCheck
