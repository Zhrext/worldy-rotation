...
-- if=buff.tip_of_the_spear.stack=3|dot.sWRapnel_bomb.ticking
local function EvaluateTargetIfRaptorStrikeST(TargetUnit)
  return (Player:BuffStack(S.TipoftheSpearBuff) == 3 or TargetUnit:DebuffUp(S.SWRapnelBombDebuff))
end
...
-- if=buff.mongoose_fury.up|focus+action.kill_command.cast_regen>focus.max-15|dot.sWRapnel_bomb.ticking|buff.wild_spirits.remains
local function EvaluateTargetIfMongooseBiteST2(TargetUnit) 
  return (Player:BuffUp(S.MongooseFuryBuff) or Player:Focus() + Player:FocusCastRegen(S.MongooseBite:ExecuteTime()) > Player:FocusMax() - 15 or TargetUnit:DebuffUp(S.SWRapnelBombDebuff) or TargetUnit:DebuffRemains(S.WildSpiritsDebuff))
end
...
local function ST()
  ...
  if S.SWRapnelBomb:IsCastable() and (S.WildfireBomb:FullRechargeTime() < 2 * Player:GCD() and Player:HasTier(28, 2) or Player:BuffUp(S.MadBombardierBuff) or (not Player:HasTier(28, 2)) and (S.WildfireBomb:FullRechargeTime() < Player:GCD() or FightRemains < 10)) then
    if Cast(S.SWRapnelBomb, nil, nil, not Target:IsSpellInRange(S.SWRapnelBomb)) then return "sWRapnel_bomb st 24"; end
  end
  ...
  -- butchery,if=active_enemies>1&!runeforge.rylakstalkers_confounding_strikes.equipped&cooldown.wildfire_bomb.full_recharge_time>spell_targets&(charges_fractional>2.5|dot.sWRapnel_bomb.ticking)
  if S.Butchery:IsReady() and (EnemyCount8ySplash > 1 and not RylakstalkersConfoundingEquipped and S.WildfireBomb:FullRechargeTime() > EnemyCount8ySplash and (S.Butchery:ChargesFractional() > 2.5 or Target:DebuffUp(S.SWRapnelBombDebuff))) then
  ...
  -- raptor_strike,target_if=max:debuff.latent_poison_injection.stack,if=buff.tip_of_the_spear.stack=3|dot.sWRapnel_bomb.ticking
  ...
  -- mongoose_bite,if=dot.sWRapnel_bomb.ticking
  if S.MongooseBite:IsReady() and (Target:DebuffUp(S.SWRapnelBombDebuff)) then
  ...
  -- wildfire_bomb,if=next_wi_bomb.sWRapnel&focus>variable.mb_rs_cost*2&dot.serpent_sting.remains>5*gcd&!set_bonus.tier28_2pc
  if S.SWRapnelBomb:IsCastable() and (Player:Focus() > MBRSCost * 2 and Target:DebuffRemains(S.SerpentStingDebuff) > 5 * Player:GCD() and not Player:HasTier(28, 2)) then
    if Cast(S.SWRapnelBomb, nil, nil, not Target:IsSpellInRange(S.SWRapnelBomb)) then return "sWRapnel_bomb st 46"; end
  end
  -- chakrams
  ...
  if RylakstalkersConfoundingEquipped then
    if S.SWRapnelBomb:IsCastable() then
      if Cast(S.SWRapnelBomb, nil, nil, not Target:IsSpellInRange(S.SWRapnelBomb)) then return "sWRapnel_bomb st 52"; end
    end
  ...
  -- mongoose_bite,target_if=max:debuff.latent_poison_injection.stack,if=buff.mongoose_fury.up|focus+action.kill_command.cast_regen>focus.max-15|dot.sWRapnel_bomb.ticking|buff.wild_spirits.remains
  if S.MongooseBite:IsReady() then
  ...
  -- wildfire_bomb,if=(next_wi_bomb.volatile&dot.serpent_sting.ticking|next_wi_bomb.pheromone|next_wi_bomb.sWRapnel&focus>50)&!set_bonus.tier28_2pc
  if (not Player:HasTier(28, 2)) then
    ...
    if S.SWRapnelBomb:IsCastable() and (Player:Focus() > 50) then
      if Cast(S.SWRapnelBomb, nil, nil, not Target:IsSpellInRange(S.SWRapnelBomb)) then return "sWRapnel_bomb st 68"; end
    end
  end
end
...
local function Cleave()
  ...
  if S.WildfireBomb:FullRechargeTime() < Player:GCD() or Player:BuffUp(S.MadBombardierBuff) or FightRemains < 5 then
    if S.SWRapnelBomb:IsCastable() then
      if Cast(S.SWRapnelBomb, nil, nil, not Target:IsSpellInRange(S.SWRapnelBomb)) then return "sWRapnel_bomb cleave 11"; end
    end
    if S.PheromoneBomb:IsCastable() then
  ...
  -- butchery,if=dot.sWRapnel_bomb.ticking&(dot.internal_bleeding.stack<2|dot.sWRapnel_bomb.remains<gcd)
  if S.Butchery:IsReady() and (Target:DebuffUp(S.SWRapnelBombDebuff) and (Target:DebuffStack(S.InternalBleedingDebuff) < 2 or Target:DebuffRemains(S.SWRapnelBombDebuff) < Player:GCD())) then
    if Cast(S.Butchery, nil, nil, not Target:IsInRange(8)) then return "butchery cleave 22"; end
  end
  -- carve,if=dot.sWRapnel_bomb.ticking&!set_bonus.tier28_2pc
  if S.Carve:IsReady() and (Target:DebuffUp(S.SWRapnelBombDebuff) and not Player:HasTier(28, 2)) then
  ...
  if (S.WildfireInfusion:IsAvailable() and (EnemyCount8ySplash > 4 and S.WildfireBomb:ChargesFractional() > 1.2 or EnemyCount8ySplash > 3 and S.WildfireBomb:ChargesFractional() > 1.4 or S.WildfireBomb:ChargesFractional() > 1.6)) then
    if S.SWRapnelBomb:IsCastable() then
      if Cast(S.SWRapnelBomb, nil, nil, not Target:IsSpellInRange(S.SWRapnelBomb)) then return "sWRapnel_bomb cleave 52"; end
    end
    if S.PheromoneBomb:IsCastable() then
  ...
  -- butchery,if=(!next_wi_bomb.sWRapnel|!talent.wildfire_infusion.enabled)&cooldown.wildfire_bomb.full_recharge_time>spell_targets%2
  if S.Butchery:IsReady() and ((not S.SWRapnelBomb:IsCastable() or not S.WildfireInfusion:IsAvailable()) and S.WildfireBomb:FullRechargeTime() > EnemyCount8ySplash / 2) then
...
local function APL()
  ...
  if AoEON() then
    if EagleUp and not Target:IsInMeleeRange(8) then
      EnemyCount8ySplash = Target:GetEnemiesInSplasWRangeCount(8)
    else
...
local function OnInit ()
  --WR.Print("Survival Hunter rotation is currently a work in progress, but has been updated for patch 9.1.5.")
end
