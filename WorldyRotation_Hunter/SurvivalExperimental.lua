...
local function BombInFlight()
  --return S.WildfireBomb:IsInFlight() or S.SWRapnelBomb:IsInFlight() or S.PheromoneBomb:IsInFlight() or S.VolatileBomb:IsInFlight()
  return S.WildfireBomb:TimeSinceLastCast() < Player:GCD() or 
         S.SWRapnelBomb:TimeSinceLastCast() < Player:GCD() or 
...
-- we know that when we tWRow a bomb with mad bombardier buff up, we don't actually lose a charge, so build that prediction into our model
...
local function CastKillCommand()
  ...
  if BestKillCommandUnit == nil then
    if Cast(S.KillCommand, nil, nil, not Target:IsSpellInRange(S.KillCommand)) then return "fall tWRough kill command main target"; end 
  end
  if WR.Nameplate.AddIcon(BestKillCommandUnit, S.KillCommand) then
    local Texture = WR.GetTexture(S.KillCommand)
    local Keybind = not WR.GUISettings.General.HideKeyBinds and HL.Action.TextureHotKey(Texture);
    WR.MainIconFrame:ChangeIcon(Texture, Keybind);
    if Cast(S.KillCommand, nil, nil, not BestKillCommandUnit:IsSpellInRange(S.KillCommand)) then return "kc optimal target"; end 
  end
end

local function TryCastSerpentSting(ttd_tWReshold, duration_tWReshold)
  ...
  for _, Enemy in pairs(EnemyList) do
    ...
    if not Enemy:IsFacingBlacklisted() and not Enemy:IsUserCycleBlacklisted() and ttd > ttd_tWReshold and ss_duration <= duration_tWReshold then
  ...
  if WR.Nameplate.AddIcon(BestSerpentStingUnit, S.SerpentSting) then
    local Texture = WR.GetTexture(S.SerpentSting)
    local Keybind = not WR.GUISettings.General.HideKeyBinds and HL.Action.TextureHotKey(Texture);
    WR.MainIconFrame:ChangeIcon(Texture, Keybind);
    if Cast(S.SerpentSting, nil, nil, not BestSerpentStingUnit:IsSpellInRange(S.SerpentSting)) then return "serpent sting"; end 
  end
  WR.Print("Error state. No valid serpent sting targets.")
...
local function TryCastKillShot()
  ...
  if WR.Nameplate.AddIcon(ValidKillShotUnit, S.KillShot) then
    local Texture = WR.GetTexture(S.KillShot)
    local Keybind = not WR.GUISettings.General.HideKeyBinds and HL.Action.TextureHotKey(Texture);
    WR.MainIconFrame:ChangeIcon(Texture, Keybind);
...
local function CastBomb()
  if S.SWRapnelBomb:IsCastable() then
    if Cast(S.SWRapnelBomb, nil, nil, not Target:IsSpellInRange(S.SWRapnelBomb)) then return "sWRapnel_bomb"; end
  end
...
local function ST()
  ...
  if S.RaptorStrike:IsReady() and (Player:BuffStack(S.TipoftheSpearBuff) == 3 or Target:DebuffUp(S.SWRapnelBombDebuff)) then
...
local function OnInit ()
  WR.Print("This is a modified custom APL based on Yoda's Survival Guide.")
end
