...
local function Precombat()
  ...
  if S.ArcaneTorrent:IsCastable() and (S.FinalReckoning:IsAvailable() and S.Seraphim:IsAvailable()) then
    if Press(S.ArcaneTorrent, not Target:IsInRange(8)) then return "arcane_torrent precombat 4"; end
  end
  -- shield_of_vengeance
  ...
  if Player:HolyPower() >= 4 and Target:IsInMeleeRange(5) then
    if S.DivineStorm:IsReady() and EnemiesCount8y >= 2 then
      if Press(S.DivineStorm) then return "divine_storm precombat 8" end
    end
    if VerdictSpell:IsReady() and EnemiesCount8y < 2 and Target:IsInMeleeRange(5) then
      if Press(VerdictSpell) then return "either verdict precombat 10" end
    end
  end
  if S.BladeofJustice:IsCastable() then
    if Press(S.BladeofJustice, not Target:IsSpellInRange(S.BladeofJustice)) then return "blade_of_justice precombat 12" end
  end
  if S.HammerofWrath:IsReady() then
    if Press(S.HammerofWrath, not Target:IsSpellInRange(S.HammerofWrath)) then return "hammer_of_wrath precombat 14" end
  end
  if S.Judgment:IsCastable() then
    if Press(S.Judgment, not Target:IsSpellInRange(S.Judgment)) then return "judgment precombat 16" end
  end
  if S.CrusaderStrike:IsCastable() then
    if Press(S.CrusaderStrike, not Target:IsInMeleeRange(5)) then return "crusader_strike 18" end
  end
end
...
local function Cooldowns()
  ...
  if S.HolyAvenger:IsCastable() and (TimeToHPG <= Player:GCDRemains() and Player:HolyPower() <= 2 and (Player:BuffUp(S.AvengingWrath) or S.Crusade:IsAvailable() and (S.Crusade:CooldownUp() or Player:BuffUp(S.CrusadeBuff)) or FightRemains < 20)) then
    if Press(S.HolyAvenger, not Target:IsInMeleeRange(8)) then return "holy_avenger cooldowns 16" end
  end
...
local function Generators()
  ...
  if S.HammerofWrath:IsReady() and (S.ZealotsParagon:IsAvailable()) then
    if Press(S.HammerofWrath, not Target:IsSpellInRange(S.HammerofWrath)) then return "hammer_of_wrath generators 2"; end
  end
  ...
  if S.WakeofAshes:IsCastable() and (Player:HolyPower() <= 2 and S.AshestoDust:IsAvailable() and (S.AvengingWrath:CooldownDown() or S.Crusade:CooldownDown())) then
    if Press(S.WakeofAshes, not Target:IsInRange(12)) then return "wake_of_ashes generators 4"; end
  end
  ...
  if S.Judgment:IsCastable() and (Target:DebuffDown(S.JudgmentDebuff) and Player:HolyPower() >= 2) then
    if Press(S.Judgment, not Target:IsSpellInRange(S.Judgment)) then return "judgment generators 8"; end
  end
  ...
  if S.WakeofAshes:IsCastable() and ((Player:HolyPower() == 0 or Player:HolyPower() <= 2 and S.BladeofJustice:CooldownRemains() > Player:GCD() * 2) and ((not S.Seraphim:IsAvailable()) or S.Seraphim:CooldownRemains() > 5) and ((not S.ExecutionSentence:IsAvailable()) or S.ExecutionSentence:CooldownRemains() > 15 or Target:TimeToDie() < 8 or EnemiesCount8y >= 5) and ((not S.FinalReckoning:IsAvailable()) or S.FinalReckoning:CooldownRemains() > 15 or FightRemains < 8) and (S.AvengingWrath:CooldownDown() or S.Crusade:CooldownDown())) then
    if Press(S.WakeofAshes, not Target:IsInRange(12)) then return "wake_of_ashes generators 10"; end
  end
  ...
  if S.Exorcism:IsCastable() then
    if Press(S.Exorcism, not Target:IsSpellInRange(S.Exorcism)) then return "exorcism generators 12"; end
  end
  -- judgment,if=!debuff.judgment.up
  if S.Judgment:IsCastable() and (Target:DebuffDown(S.JudgmentDebuff)) then
    if Press(S.Judgment, not Target:IsSpellInRange(S.Judgment)) then return "judgment generators 14"; end
  end
  -- hammer_of_wrath
  if S.HammerofWrath:IsReady() then
    if Press(S.HammerofWrath, not Target:IsSpellInRange(S.HammerofWrath)) then return "hammer_of_wrath generators 16"; end
  end
  ...
  if S.Consecration:IsCastable() and (Target:DebuffDown(S.ConsecrationDebuff) and EnemiesCount8y >= 2) then
    if Press(S.Consecration, not Target:IsInMeleeRange(8)) then return "consecration generators 20"; end
  end
  ...
  if S.CrusaderStrike:IsCastable() and (S.CrusaderStrike:ChargesFractional() >= 1.75 and (Player:HolyPower() <= 2 or Player:HolyPower() <= 3 and S.BladeofJustice:CooldownRemains() > Player:GCD() * 2 or Player:HolyPower() == 4 and S.BladeofJustice:CooldownRemains() > Player:GCD() * 2 and S.Judgment:CooldownRemains() > Player:GCD() * 2)) then
    if Press(S.CrusaderStrike, not Target:IsInMeleeRange(5)) then return "crusader_strike generators 22"; end
  end
  -- call_action_list,name=finishers
  local ShouldReturn = Finishers(); if ShouldReturn then return ShouldReturn; end
  -- consecration,if=!consecration.up
  if S.Consecration:IsCastable() and (Target:DebuffDown(S.ConsecrationDebuff)) then
    if Press(S.Consecration, not Target:IsInMeleeRange(8)) then return "consecration generators 24"; end
  end
  -- crusader_strike
  if S.CrusaderStrike:IsCastable() then
...
local function ESFRActive()
  ...
  if S.DivineToll:IsCastable() and (Player:HolyPower() <= 2) then
    if Press(S.DivineToll, not Target:IsInRange(30)) then return "divine_toll es_fr_active 4"; end
  end
  ...
  if S.WakeofAshes:IsCastable() and (Player:HolyPower() <= 2 and (Target:DebuffUp(S.FinalReckoning) and Target:DebuffRemains(S.FinalReckoning) < Player:GCD() * 2 and (not S.DivineResonance:IsAvailable()) or Target:DebuffUp(S.ExecutionSentence) and Target:DebuffRemains(S.ExecutionSentence) < Player:GCD() or EnemiesCount8y >= 5 and S.DivineResonance:IsAvailable() and S.ExecutionSentence:IsAvailable())) then
    if Press(S.WakeofAshes, not Target:IsInRange(12)) then return "wake_of_ashes es_fr_active 6"; end
  end
  ...
  if S.BladeofJustice:IsCastable() and (S.Expurgation:IsAvailable() and ((not S.DivineResonance:IsAvailable()) and Player:HolyPower() <= 3 or Player:HolyPower() <= 2)) then
    if Press(S.BladeofJustice, not Target:IsSpellInRange(S.BladeofJustice)) then return "blade_of_justice es_fr_active 8"; end
  end
  -- judgment,if=!debuff.judgment.up&holy_power>=2
  if S.Judgment:IsCastable() and (Target:DebuffDown(S.JudgmentDebuff) and Player:HolyPower() >= 2) then
    if Press(S.Judgment, not Target:IsSpellInRange(S.Judgment)) then return "judgment es_fr_active 10"; end
  end
  -- call_action_list,name=finishers
  local ShouldReturn = Finishers(); if ShouldReturn then return ShouldReturn; end
  -- wake_of_ashes,if=holy_power<=2
  if S.WakeofAshes:IsCastable() and (Player:HolyPower() <= 2) then
    if Press(S.WakeofAshes, not Target:IsInRange(12)) then return "wake_of_ashes es_fr_active 12"; end
  end
  -- blade_of_justice,if=holy_power<=3
  if S.BladeofJustice:IsCastable() and (Player:HolyPower() <= 3) then
    if Press(S.BladeofJustice, not Target:IsSpellInRange(S.BladeofJustice)) then return "blade_of_justice es_fr_active 14"; end
  end
  -- judgment,if=!debuff.judgment.up
  if S.Judgment:IsCastable() and (Target:DebuffDown(S.Judgment)) then
    if Press(S.Judgment, not Target:IsSpellInRange(S.Judgment)) then return "judgment es_fr_active 16"; end
  end
  -- hammer_of_wrath
  if S.HammerofWrath:IsReady() then
    if Press(S.HammerofWrath, not Target:IsSpellInRange(S.HammerofWrath)) then return "hammer_of_wrath es_fr_active 18"; end
  end
  -- crusader_strike
  if S.CrusaderStrike:IsCastable() then
    if Press(S.CrusaderStrike, not Target:IsInMeleeRange(5)) then return "crusader_strike es_fr_active 20"; end
  end
  -- arcane_torrent
  if S.ArcaneTorrent:IsCastable() then
    if Press(S.ArcaneTorrent, not Target:IsInRange(8)) then return "arcane_torrent es_fr_active 22"; end
  end
  -- exorcism
  if S.Exorcism:IsCastable() then
    if Press(S.Exorcism, not Target:IsSpellInRange(S.Exorcism)) then return "exorcism es_fr_active 24"; end
  end
  -- consecration
  if S.Consecration:IsCastable() then
    if Press(S.Consecration, not Target:IsInMeleeRange(8)) then return "consecration es_fr_active 26"; end
  end
end
...
local function ESFRPooling()
  ...
  if S.Seraphim:IsReady() and (Player:HolyPower() == 5 and ((not S.FinalReckoning:IsAvailable()) or S.FinalReckoning:CooldownRemains() <= Player:GCD() * 3) and ((not S.ExecutionSentence:IsAvailable()) or S.ExecutionSentence:CooldownRemains() <= Player:GCD() * 3 or S.FinalReckoning:IsAvailable())) then
    if Press(S.Seraphim, not Target:IsInMeleeRange(8)) then return "seraphim es_fr_pooling 2"; end
  end
  ...
  if S.HammerofWrath:IsReady() and (S.VanguardsMomentum:IsAvailable()) then
    if Press(S.HammerofWrath, not Target:IsSpellInRange(S.HammerofWrath)) then return "hammer_of_wrath es_fr_pooling 4"; end
  end
  ...
  if S.WakeofAshes:IsCastable() and (Player:HolyPower() <=2 and S.AshestoDust:IsAvailable() and (S.Crusade:CooldownDown() or S.AvengingWrath:CooldownDown())) then
    if Press(S.WakeofAshes, not Target:IsInRange(12)) then return "wake_of_ashes es_fr_pooling 6"; end
  end
  -- blade_of_justice,if=holy_power<=3
  if S.BladeofJustice:IsCastable() and (Player:HolyPower() <= 3) then
    if Press(S.BladeofJustice, not Target:IsSpellInRange(S.BladeofJustice)) then return "blade_of_justice es_fr_pooling 8"; end
  end
  -- judgment,if=!debuff.judgment.up
  if S.Judgment:IsCastable() and (Target:DebuffDown(S.JudgmentDebuff)) then
    if Press(S.Judgment, not Target:IsSpellInRange(S.Judgment)) then return "judgment es_fr_pooling 10"; end
  end
  -- hammer_of_wrath
  if S.HammerofWrath:IsReady() then
    if Press(S.HammerofWrath, not Target:IsSpellInRange(S.HammerofWrath)) then return "hammer_of_wrath es_fr_pooling 12"; end
  end
  -- crusader_strike,if=cooldown.crusader_strike.charges_fractional>=1.75&(holy_power<=2|holy_power<=3&cooldown.blade_of_justice.remains>gcd*2|holy_power=4&cooldown.blade_of_justice.remains>gcd*2&cooldown.judgment.remains>gcd*2)
  if S.CrusaderStrike:IsCastable() and (S.CrusaderStrike:ChargesFractional() >= 1.75 and (Player:HolyPower() <= 2 or Player:HolyPower() <= 3 and S.BladeofJustice:CooldownRemains() > Player:GCD() * 2 or Player:HolyPower() == 4 and S.BladeofJustice:CooldownRemains() > Player:GCD() * 2 and S.Judgment:CooldownRemains() > Player:GCD() * 2)) then
    if Press(S.CrusaderStrike, not Target:IsInMeleeRange(5)) then return "crusader_strike es_fr_pooling 14"; end
  end
  ...
  if S.Seraphim:IsReady() and ((not S.FinalReckoning:IsAvailable()) and S.ExecutionSentence:CooldownRemains() <= Player:GCD() * 3) then
    if Press(S.Seraphim, not Target:IsInMeleeRange(8)) then return "seraphim es_fr_pooling 16"; end
  end
  -- call_action_list,name=finishers
  local ShouldReturn = Finishers(); if ShouldReturn then return ShouldReturn; end
  -- crusader_strike
  if S.CrusaderStrike:IsCastable() then
    if Press(S.CrusaderStrike, not Target:IsInMeleeRange(5)) then return "crusader_strike es_fr_pooling 18"; end
  end
  -- arcane_torrent,if=holy_power<=4
  if S.ArcaneTorrent:IsCastable() and (Player:HolyPower() <= 4) then
    if Press(S.ArcaneTorrent, not Target:IsInRange(8)) then return "arcane_torrent es_fr_pooling 20"; end
  end
  -- exorcism
  if S.Exorcism:IsCastable() then
    if Press(S.Exorcism, not Target:IsSpellInRange(S.Exorcism)) then return "exorcism es_fr_pooling 22"; end
  end
  ...
  if S.Seraphim:IsReady() and (((not S.FinalReckoning:IsAvailable()) or S.FinalReckoning:CooldownRemains() <= Player:GCD() * 3) and ((not S.ExecutionSentence:IsAvailable()) or S.ExecutionSentence:CooldownRemains() <= Player:GCD() * 3 or S.FinalReckoning:IsAvailable())) then
    if Press(S.Seraphim, not Target:IsInMeleeRange(8)) then return "seraphim es_fr_pooling 24"; end
  end
  -- consecration
  if S.Consecration:IsCastable() then
    if Press(S.Consecration, not Target:IsInMeleeRange(8)) then return "consecration es_fr_pooling 26"; end
  end
end
