...
local function Defensives()
  ...
  if S.WordofGlory:IsReady() and (Player:HealthPercentage() <= Settings.Protection.WordofGloryHP and not Player:HealingAbsorbed()) then
    ...
    if (Player:BuffRemains(S.ShieldoftheRighteousBuff) >= 5 or Player:BuffUp(S.DivinePurposeBuff) or Player:BuffUp(S.ShiningLightFreeBuff)) then
      ...
      if WR.CastAnnotated(S.WordofGlory, false, "KITE") then return "word_of_glory defensive 10"; end
    end
  end
    ...
    if Player:HealthPercentage() > 90 and Player:IsInParty() and not Player:IsInRaid() then
      for _, Char in pairs(Unit.Party) do
        if Char:Exists() and Char:IsInRange(40) and Char:HealthPercentage() <= 80 then
          if WR.CastAnnotated(S.WordofGlory, false, Char:Name()) then return "word_of_glory standard party 28"; end
        end
      end
...
local function AutoBind()
  -- Spell Binds
  WR.Bind(S.Consecration)
  -- Abilities
  WR.Bind(S.Judgment)
  -- Talents
  WR.Bind(S.ArdentDefender)
  WR.Bind(S.AvengersShield)
  WR.Bind(S.BastionofLight)
  WR.Bind(S.BlessedHammer)
  WR.Bind(S.CrusadersJudgment)
  WR.Bind(S.DivineToll)
  WR.Bind(S.EyeofTyr)
  WR.Bind(S.GuardianofAncientKings)
  WR.Bind(S.HammeroftheRighteous)
  WR.Bind(S.MomentofGlory)
  WR.Bind(S.CrusaderStrike)
  WR.Bind(S.DivineShield)  
  WR.Bind(S.FlashofLight)  
  WR.Bind(S.HammerofJustice)
  WR.Bind(S.HandofReckoning)
  WR.Bind(S.Judgment)       
  WR.Bind(S.Rebuke)         
  WR.Bind(S.ShieldoftheRighteous)
  WR.Bind(S.WordofGlory)         
  -- Talents
  WR.Bind(S.AvengingWrath)       
  WR.Bind(S.HammerofWrath)       
  WR.Bind(S.HolyAvenger)         
  WR.Bind(S.LayonHands)          
  WR.Bind(S.Seraphim)            
  WR.Bind(S.ZealotsParagon)
